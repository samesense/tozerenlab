kmeansHM<-function(expdata,k,niterations,trueCI,cluster=NULL){

require(gplots)
require(SparseM)

#Plot a heatmap of the posterior pairwise distribution of expression data
#clustered with kmeans clustering

#Inputs
#expdata[n,d] = expression data, n = number of samples, d = number of genes
#k = optimal number of clusters determined using the gap statistic 
#niterations = number of random starts for kmeans algorithm 
#trueCI[n] = phenotype information
#cluster[n] = cluster indicator, if not null skips the kmeans step 

#Outputs
#writes heatmap plot to file kmeansMap.pdf
#List with two components
#kClust[n] = cluster indicator
#CI[n] = true classification



if(is.null(cluster)){
	kmeansClusters<-kmeansClust(expdata,k,niterations) #niterations X nsamples
}else{
	kmeansClusters<-cluster
}
nsamples<-dim(expdata)[1]

#resort classes by nsamples/class
ns_class<-table(trueCI) #number of samples/class
classes<-suppressWarnings(ns_class@dimnames$trueCI) #class labels in same order as ns_class
newCI_IDX<-rep(0,nsamples)
count<-0
...
newCluster<-kmeansClusters[,newCI_IDX]

#pairwise posterior distribution
pwise<-as.matrix.csr(0,nrow=nsamples,ncol=nsamples)

for(i in 1:nrow(newCluster)){
	pwise<-pwise+as.matrix.csr(outer(newCluster[i,],newCluster[i,],'=='))
}
pwise<-pwise/nrow(newCluster)
PD<-as.matrix(pwise)
...
dev.print(device=pdf, file='kmeansMap.pdf'))
return(list(kClust=kmeansClusters,CI=trueCI))
}
...
kmeansClust<-function(expdata,k,niterations){

kmeansCluster<-matrix(0,nrow=niterations,ncol=nrow(expdata))
for(i in 1:niterations){
kmeansCluster[i,]<-kmeans(expdata,k,iter.max=10,nstart=10)$cluster
}
