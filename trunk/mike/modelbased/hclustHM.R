hclustHM<-function(expdata,k,trueCI){
...
#Outputs
#writes heatmap plot to file hclustMap.pdf
#List with two components
#hcClust[n] = cluster indicator
#CI[n] = true classification

#Classify samples by hierarchical clustering
hcClusters<-hclust(dist(expdata),method='complete')
hcClusters<-cutree(hcClusters,k) 

#resort samples by nsamples/class
nsamples<-dim(expdata)[1] #number of samples
...
newCluster<-hcClusters[newCI_IDX]

#pairwise posterior distribution 
pwise<-as.matrix.csr(outer(newCluster,newCluster,'=='))
PD<-as.matrix(pwise)
...
sepvec<-rep(0,length(classLabels)-1) #white lines separating classes
colbar<-rep(NA,length(trueCI)) #color bar
