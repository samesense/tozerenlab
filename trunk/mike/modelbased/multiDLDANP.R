multiDLDANP<-function(data,n.iterations,nfeatures,classes,Key=NULL,filename=NULL,random=F){
...
for(i in 1:length(classes)){
	if(random){
		classif<-multiRANDOM(classes[i],data,nfeatures,lset)
	}else{
		classif<-multiDLDA.classification(classes[i],data,nfeatures,lset)
	}
	DLDA<-c(DLDA,list(classif))
}
